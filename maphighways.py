...
class MapHighways(object):
	...
	def DrawProcessing(self, bounds, zoom, hints, layer, DrawCallback):
		#bounds left,bottom,right,top
		...
		for obj in highwayNetwork:
			...
			for node in wayNodes:
				...
				x = (nodePos[1] - bounds[0]) / tileLonWidth
				y = (nodePos[0] - bounds[1]) / tileLonHeight
				#print nodePos, x, y
				xyPairs.append(x)
				xyPairs.append(y)

			li = Line(points=xyPairs, width=10)
			DrawCallback(li)
